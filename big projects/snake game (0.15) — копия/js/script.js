...
let blockSize = 10;
let extraBlockSize = 8;
let extraObstacleSaver = true;
let rectSize = 20;
...
let lives = 7;
let lifeObstacleChance = Math.random;
let toggleVar = true;
let randomNumber;
...
Block.prototype.drawExtraCircle = function(color) {
	let centerX = this.col * blockSize + blockSize / 2;
	let centerY = this.row * blockSize + blockSize / 2;
	context.fillStyle = color;
	//circle(centerX, centerY, extraBlockSize / 2, true);
	circle(centerX, centerY, extraBlockSize / 2, true);
	 //else if (random >= 0.5){
	//circle(900, 900, extraBlockSize / 2, true);	
	//} 
};
...
Snake.prototype.move = function () {
	...
	if (newHead.equal(apple.position)) {
		...
		extraLifeObstacle.move();
		randomNumber = Math.random();
		//extraObstacleSaver = true;
		...
		killObstacle.move();
		decreaseObstacle.move(); 
		killSound = new sound("sound/hit.wav");
	    killSound.play();
		extraLifeObstacle.move();
		randomNumber = Math.random();
		//extraObstacleSaver = true;
	...
	} else if (newHead.equal(extraLifeObstacle.position)) {
        lives++;
		apple.move();
		killObstacle.move();
		decreaseObstacle.move();
...
let MovingKillObstacle = function(){
	this.position = new Block(15,19);
	this.direction = "right";
	this.nextDirection = "right";
};

MovingKillObstacle.prototype.draw = function () {
	this.position.drawSquare("Red");
};

let ExtraLifeObstacle = function(){
	this.position = new Block(11,15);
};

ExtraLifeObstacle.prototype.draw = function () {
	this.position.drawExtraCircle("#b800e6");
};
...
MovingKillObstacle.prototype.move = function(){
	let newPlace;
	let random = Math.random();
	let head = this.position;
	
	if(random < 0.25 && this.direction != "left"){
		this.direction = "right"
	} else if (random >= 0.25 && random < 0.5 && this.direction != "right"){
		this.direction = "left"
	} else if (random >= 0.5 && random < 0.75 && this.direction != "up"){
		this.direction = "down"
	...
	if (this.direction === "right") {
		newPlace = new Block(head.col + 1, head.row);
	} else if (this.direction === "down") {
		newPlace = new Block(head.col, head.row + 1);
	} else if (this.direction === "left") {
		newPlace = new Block(head.col - 1, head.row);
	} else if (this.direction === "up") {
		newPlace = new Block(head.col, head.row - 1);
...
MovingKillObstacle.prototype.setDirection = function (newDirection) {
	if (this.direction === "up" && newDirection === "down") {
		return;
	} else if (this.direction === "right" && newDirection === "left") {
		return;
...
ExtraLifeObstacle.prototype.move = function () {
	let randomCol = Math.floor(Math.random() * (widthInBlocks - 2)) + 1;
	let randomRow = Math.floor(Math.random() * (heightInBlocks - 2)) + 1;
	this.position = new Block(randomCol, randomRow);
};
...
let apple = new Apple();
let	extraLifeObstacle = new ExtraLifeObstacle();
let movingKillObstacle = new MovingKillObstacle();
...
function toggleNav(){
	...
	if (toggleVar == true ){
		document.getElementById("control").style.width = "500px";
		document.getElementById("flexContainer").style.display = "flex";
		document.getElementById("video").hidden = false;
		//document.getElementById("jsButton").style.visibility = "visible";
		toggleVar = false;
		...
		//document.getElementById("jsButton").style.visibility = "hidden";
		toggleVar = true;
}
}
...
function startGame(){
	...
	let increaseDifficulty = function(){ 
	if(intervalTimer === 100 && score === 1){
    intervalTimer = 60;
	} else if (intervalTimer === 60 && score === 2){
	intervalTimer = 20;
	} else if (intervalTimer === 20 && score === 3){
    intervalTimer = 10;
	} else if (intervalTimer === 50 && score === 1){
	intervalTimer = 25;
	} else if (intervalTimer === 25 && score === 2){
	intervalTimer = 12;
    } else if (intervalTimer === 12 && score === 3){
	 intervalTimer = 7; 
	} else { 
	...
	killObstacle.draw();
	if(extraObstacleSaver === true){
	extraLifeObstacle.draw(); 
	};
	movingKillObstacle.move();
	movingKillObstacle.draw();
	apple.draw();
	drawScore();
	drawLives();
	drawBorder();
